(function() {
    ...
    function jhiAlertErrorController ($scope, AlertService, $rootScope, $translate) {
        ...
        var cleanHttpErrorListener = $rootScope.$on('assessoriaTorrellesApp.httpError', function (event, httpResponse) {
            var i;
            ...
            case 400:
                var errorHeader = httpResponse.headers('X-assessoriaTorrellesApp-error');
                var entityKey = httpResponse.headers('X-assessoriaTorrellesApp-params');
                if (errorHeader) {
                    ...
                    for (i = 0; i < httpResponse.data.fieldErrors.length; i++) {
                        ...
                        var fieldName = $translate.instant('assessoriaTorrellesApp.' + fieldError.objectName + '.' + convertedField);
